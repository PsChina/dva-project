import * as service from '../../services/tms/terminalTxnAuth';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'terminalTxnAuthManage';
const objectId = 'companyId';
const enterPath = '/tms/terminalManage/terminalTxnAuthManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * refreshCache({ payload }, { call, put, select }){
    const res = yield call(service.refreshCache, { ...payload });
    console.log('res jie guowei:',res);
    const detail = parseResponse(res);
    console.log('detail zhi',detail);
    if(detail.rspCod === '200'){
      callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
    }else{
      callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
  ...
  reducers: {
    ...
    updateStatusSuccess(state, action) {
      ...
      const status = action.payload.txnStatus;
      const newTableList = state.tableList.map((item) => {
        if (ids.indexOf(item[objectId]) !== -1) {
          return { ...item, txnStatus: status };
        }
        return item;
      });
