import * as service from '../../services/tms/terminalAgent';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'terminalAgentManage';
const objectId = 'agtId';
const enterPath = '/tms/terminalBindManage/terminalAgentManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          dispatch({ type: 'queryList', payload: { tableParam: { currentPage: 1, agtLv: '1' } } });
        }
      });
    },
  },
  effects: {
    ...
    * queryTermList({ payload }, { call, put }) {
      yield put({
        type: 'updateState',
        payload: { ...payload, termTableSelects: [], termTableLoading: true, termPayLoadAgtId: payload.data },
      });
    ...
    * queryUnBindTermList({ payload }, { call, put }) { 
      yield put({
        type: 'updateState',
        payload: { ...payload, addModalTableSelects: [], addModalTableLoading: true, addModalVisible: true, addModalPayLoadAgtId: payload.data },
      });
      const res = yield call(service.queryUnBindTermList, { ...payload });
      const detail = parseResponse(res);
      // debug
      for( let key in detail['rspList'] ){
        let value = detail['rspList'][key];
        if(!value){
          detail['rspList'][key] = {};
        }
      }
    ...
    * addOne({ payload }, { call, put }) {
      ...
      if (detail.rspCod === '200') {
        ...
        yield put({
          type: 'updateState',
          payload: { termTableSelects: [], termTableLoading: true, termPayLoadAgtId: payload.data },
        });
        const res = yield call(service.queryTermList, { ...payload });
