import * as service from '../../services/bas/procedure';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'procedureManage';
const objectId = 'nodeno';
const enterPath = '/bas/workflow/procedureManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'findDDLForAddNode', payload: { systemno: '000' } });
        }
      });
    },
  },
  effects: {
    ...
    * updateStatus({ payload }, { call, put }) {
    ...
    // query工作流和岗位方法
    * findDDLForAddNode({ payload }, { call, put }) {
      const res = yield call(service.findDDLForAddNode, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { modelMainOptionsData: detail.rspData.modelMainList, positionOptionsData: detail.rspData.positionList },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
